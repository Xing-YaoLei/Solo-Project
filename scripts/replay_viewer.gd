...
func _refresh_list() -> void:
	for child in replay_list.get_children():
		child.queue_free()
	_replays = ReplayManager.get_replays()
	if _replays.is_empty():
		...
		_clear_playback()
		return
	for i in _replays.size():
		var replay: Dictionary = _replays[i]
		var btn = Button.new()
		btn.text = "%s - %s" % [replay.get("patient_id", "未知"), replay.get("timestamp", "")]
...
func _show_playback(index: int) -> void:
	for child in playback_steps.get_children():
		child.queue_free()
	if index < 0 or index >= _replays.size():
		hesitation_label.text = ""
		return
	var replay: Dictionary = _replays[index]
	...
	for i in steps.size():
		...
		time_label.text = "%.1fs" % step.get("elapsed", 0.0)
		time_label.add_theme_color_override("font_color", Color(0.7, 0.7, 0.8))
		row.add_child(time_label)

		if is_hesitation:
			var hes_label = Label.new()
			var resp_time = _get_hesitation_response_time(hesitations, i)
			hes_label.text = "犹豫 %.1fs" % resp_time
			hes_label.add_theme_color_override("font_color", Color(1.0, 0.9, 0.2))
			row.add_child(hes_label)

		playback_steps.add_child(row)

	hesitation_label.text = "犹豫点: %d / %d 步" % [hesitation_count, steps.size()]
	play_button.disabled = false

func _get_hesitation_response_time(hesitations: Array, step_index: int) -> float:
	if step_index < hesitations.size():
		return hesitations[step_index].get("response_time", 0.0)
	return 0.0

func _on_play_pressed() -> void:
	if _selected_index < 0 or _selected_index >= _replays.size():
		return
	if _is_playing:
		_stop_playback()
		return
