...
class StorePoint(Base):
    ...
    status = Column(Enum(PointStatus), default=PointStatus.ACTIVE)
...
class CleaningRecord(Base):
    ...
    source_channel = Column(Enum(SourceChannel), default=SourceChannel.ROUTINE_INSPECTION)
    status = Column(Enum(CleaningStatus), default=CleaningStatus.DRAFT)
    ...
    close_reason = Column(Enum(CloseReason))
...
class StatusLog(Base):
    ...
    from_status = Column(Enum(CleaningStatus))
    to_status = Column(Enum(CleaningStatus), nullable=False)
