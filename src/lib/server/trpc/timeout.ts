	...
	for (const order of overdueOrders) {
		...
		if (escalationRules.length > 0) {
			...
			if (rule.escalateToUserId) {
				...
				if (targetUsers.length > 0) {
					newHandlerId = targetUsers[0].id;
					newHandlerName = targetUsers[0].realName || targetUsers[0].username;
				}
			}
			escalateInfo = `按规则「${rule.name}」升级至: ${newHandlerName}`;
		} else {
			escalateInfo = `超时未处理，已标记为超时待办`;
		}
		...
		await db
			...
			.set({
				...
				status: 'escalated',
				dueAt: await getDueAt('escalated'),
				updatedAt: now
			})
		...
		await db.insert(refundLogTable).values({
			...
			actionDetail: `处理超时触发: 截止时间 ${order.dueAt?.toISOString()}，${escalateInfo}`,
