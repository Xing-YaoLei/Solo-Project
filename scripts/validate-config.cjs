    ...
    try {
        ...
        switch (file) {
            case 'LevelsConfig.json':
                totalLevels = (json.levels || []).length;
            ...
            case 'TasksConfig.json':
                ...
                Object.values(json.tasks || {}).forEach((t) => {
                    (t.actionOptions || []).forEach((o) => {
            ...
            case 'CluesConfig.json':
                ...
                Object.values(json.clues || {}).forEach((c) => {
            ...
            case 'QuestionSetsConfig.json':
                ...
                sets.forEach(sid => {
                    totalQuestions += (json.questionSets[sid].questions || []).length;
                });
...
const clues = JSON.parse(fs.readFileSync(cluesPath, 'utf-8')).clues || {};
const prescriptions = JSON.parse(fs.readFileSync(prescPath, 'utf-8')).prescriptions || {};
const scenes = JSON.parse(fs.readFileSync(scenesPath, 'utf-8')).scenes || {};

let brokenLinks = [];
let linkedTasks = new Set();
let linkedClues = new Set();
let linkedPrescriptions = new Set();
let linkedScenes = new Set();
...
const prescCount = Object.keys(prescriptions).length;
const sceneCount = Object.keys(scenes).length;

console.log(`  关卡引用任务覆盖率: ${linkedTasks.size}/${taskCount} (${((linkedTasks.size / Math.max(1, taskCount)) * 100).toFixed(1)}%)`);
console.log(`  任务引用线索覆盖率: ${linkedClues.size}/${clueCount} (${((linkedClues.size / Math.max(1, clueCount)) * 100).toFixed(1)}%)`);
console.log(`  关卡引用处方: ${linkedPrescriptions.size}/${prescCount}`);
console.log(`  关卡引用场景: ${linkedScenes.size}/${sceneCount}`);
