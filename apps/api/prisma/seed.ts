import { PrismaClient, UserRole, RefundStatus, ResponsibilityParty, ReminderChannel } from '@prisma/client';
...
async function main() {
    ...
    data: [
      {
        ...
        responsibility: ResponsibilityParty.PLATFORM,
        priority: 10,
      },
      {
        name: '物流问题-物流责任',
        problemTags: ['物流配送延迟'],
        responsibility: ResponsibilityParty.LOGISTICS,
        priority: 8,
      },
      {
        name: '商家责任',
        problemTags: ['错发漏发', '规格不符', '虚假宣传'],
        responsibility: ResponsibilityParty.MERCHANT,
        priority: 9,
      },
      {
        name: '华东区指派',
  ...
  const sampleOrders = [
    ...
    },
    {
      ...
      responsibility: ResponsibilityParty.SUPPLIER,
      assigneeId: operators[1].id,
  ...
  for (const orderData of sampleOrders) {
    ...
    if (order.status === RefundStatus.CLOSED) {
      await prisma.refundTimeline.create({
        data: {
          refundOrderId: order.id,
          action: 'CLOSED',
          oldStatus: RefundStatus.REVIEWING,
