...
export async function ensureDbInitialized() {
	...
	try {
		const result = await client`SELECT COUNT(*) as cnt FROM "user"`;
		const hasUsers = Number(result[0].cnt) > 0;
		if (hasUsers) {
			await client.end();
			return;
		}
