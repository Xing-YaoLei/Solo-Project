import sys
import os

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from datetime import date, timedelta

from sync_tasks.celery_app import app
from sync_tasks.base_sync import batch_manager, upsert_records, get_date_range
from sync_tasks.data_sources import mock_fetch_schedules
from utils.db_adapter import CourseSchedule
from config import sync_config


@app.task(bind=True, name="sync_tasks.tasks.sync_schedules", autoretry_for=(Exception,),
          retry_kwargs={"max_retries": 3, "countdown": 5})
def sync_schedules(self, start_date_str: str = None, end_date_str: str = None):
    start_date, end_date = get_date_range(
        date.fromisoformat(start_date_str) if start_date_str else None,
        date.fromisoformat(end_date_str) if end_date_str else None,
        days=60
    )

    with batch_manager("schedule", start_date, end_date) as ctx:
        batch_no = ctx["batch_no"]
        session = ctx["session"]

        all_records = mock_fetch_schedules(start_date, end_date)

        batch_size = sync_config.BATCH_SIZE
        for i in range(0, len(all_records), batch_size):
            chunk = all_records[i:i + batch_size]
            upsert_records(
                session, CourseSchedule, chunk,
                unique_keys=["schedule_no"],
                batch_no=batch_no,
                ctx=ctx
            )

    return {
        "batch_no": batch_no,
        "total": ctx["total_count"],
        "success": ctx["success_count"],
        "fail": ctx["fail_count"],
        "date_range": [start_date.isoformat(), end_date.isoformat()],
    }
